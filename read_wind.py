...
class grib_reader(object):
    ...
    def run(self):
        ...
        # grib2_files = [os.path.join(gribing.process_path, row) for row in glob.glob1(gribing.process_path, "201902wnd10m.gdas.201902.grib2")]
        grib2_files = [os.path.join(gribing.process_path, row) for row in glob.glob1(gribing.process_path, "201903wnd10m.gdas.201903.grib2")]
        for file_ in grib2_files:
            print(file_, "is being processed")
            temp_grib_dataset = gdal.Open(file_)
            counter, CC, Start_date = 0, 0, datetime.datetime.strptime('20190201', '%Y%m%d')
            ...
            for raster in range(1, temp_grib_dataset.RasterCount + 1):
                if raster >= 3:
                    ...
                    if 13 <= counter < 15:
                        ...
                        writer = pd.ExcelWriter(os.path.join(os.path.splitext(file_)[0] + "__" + str(raster) + "_" +flag + ".xlsx"),
                                                engine='xlsxwriter')
                        print((Start_date + datetime.timedelta(hours=ceil(CC))).strftime("%Y%m%d_%H%M"))
                        workbook = writer.book,

                        # df = pd.DataFrame(data=temp_grib_dataraster.ReadAsArray()/100.0)
                        df = pd.DataFrame(data=temp_grib_dataraster.ReadAsArray())
                        # import osr
                        # f_name = "d_sil.gtiff"
                        ...
                        df2.to_excel(
                            writer, sheet_name = (Start_date + datetime.timedelta(hours=ceil(CC))).strftime("%Y%m%d_%H%M"),
                            startcol=1,
                            startrow=1
                        )
                        writer.close()
