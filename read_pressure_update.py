...
class grib_reader(object):
    ...
    def run(self):
        ...
        grib2_files = [os.path.join(gribing.process_path, row) for row in glob.glob1(gribing.process_path, "201903pressfc.gdas.201903*.grib2")]
        for file_ in grib2_files:
            print(file_, "is being processed")
            temp_grib_dataset = gdal.Open(file_)
            counter, CC, Start_date = 0, 0, datetime.datetime.strptime('20190301', '%Y%m%d')
